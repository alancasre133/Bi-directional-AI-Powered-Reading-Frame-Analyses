...
def createJSON(Data_rows,op):
  if createdjson("AnalyzedORFS\\matchs.json")==2 and createdjson("AnalyzedORFS\\exported_json_data.xlsx")!=1:
    ...
    excel =pd.read_excel("AnalyzedORFS\\exported_json_data.xlsx")
    index=0
    Row_Selected = {'OrganismName': (Data_rows.iloc[op-1])['Definition'],'Nucleotide Sequence':(Data_rows.iloc[op-1])['NucleotideSequence'],'ORFanalyzed':(Data_rows.iloc[op-1])['ORF'],'SpecieName':[],'Ancestor':[],'ORFSequenceNucleotide':[],'ORFSequenceAminoacid':[],'Direction':[],'Startcodon':[],'Endcodon':[],'Startaminoacid':[],'Endaminoacid':[],'AncestorSequence':[]}
    ...
    for i in range(0,len(Data_rows)):
      if i!=(op-1):
        ...
        if x>=0.35:
          index=0
          print(list(Data_rows.iloc[i])[index])
          for m in names:
            print(list(Data_rows.iloc[i])[index])
